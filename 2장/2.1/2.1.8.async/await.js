...
async function findAndSaveUser() {
    try {
        ...
        const updatedUser = await user.save();  // 저장 함수 호출
        console.log('업데이트된 사용자:', updatedUser);

        const foundUser = await User.findOne({ gender: 'm' });
        if (foundUser.gender === 'male') {
            console.log('성별이 남자인 사용자:', foundUser);
        } else if (foundUser.gender === 'female') {
            console.log('성별이 여자인 사용자:', foundUser);
        } else {
